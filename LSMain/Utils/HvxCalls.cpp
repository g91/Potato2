...
namespace Hvx
{
	...
	namespace SysCalls
	{
		...
		static HvxCall HvxEnableTimeBase(BYTE enabled)
		{
			__asm
			{
				li	r0, 0x42
		...
		static HvxCall HvxSetState(DWORD mode)
		{
			__asm {
				li      r0, 0x76
				sc
	...
	QWORD HvShadowboot()
	{
		...
		return SysCalls::HvxShadowboot();
	}

	QWORD HvBlowFuses(QWORD inp1)
	{
		return SysCalls::HvxBlowFuses(inp1);
	}

	QWORD HvKeysGetStatus()
	{
		return SysCalls::HvxKeysGetStatus();
	}
	...
	{
		return SysCalls::HvxEnableTimeBase(enabled);
	}

	QWORD HvGetProtectedFlags()
	{
		return SysCalls::HvxGetProtectedFlags();
	}
	...
	{
		return SysCalls::HvxSetState(mode);
	}
	...
	QWORD ExecutePayload(PBYTE pbPayload, DWORD cbPayload, DWORD EntryPoint, QWORD Arg1, QWORD Arg2, QWORD Arg3, QWORD Arg4)
	{
		...
		if (physCheck == 0)
		{
			if ((physPayload & 0xFFFFFFFF) <= 0x1FFFFFFF)
			{
				...
				Hvx::HvPokeDWORD(0x617C, 0x38600001); // li r3, 1
				...
				ret = HvKeysExecute(MmGetPhysicalAddress(bPayload), cBufferSize, Arg1, Arg2, Arg3, Arg4);

				// restore the rsa check
				Hvx::HvPokeDWORD(0x617C, 0x48005245); // bl XeCryptBnQwBeSigVerify
			}
			else
			{
		...
		else
		{
			...
			printf("[physCheck: 0x%016llX][cBufferSize: 0x%X]\n", physCheck, cBufferSize);
			ret = -1;
		}
		...
		if (!Utilities::CWriteFile("Hdd:\\KeysExecuteRetBuf.bin", bPayload, cBufferSize))
	...
	HRESULT InitializeHvHash()
	{
		...
		// Copy over our expansion data
		ZeroMemory(pPhysExp, 0x1000);
		memcpy(pPhysExp, HvHashExp, sizeof(HvHashExp));
	...
	{
		return HvExpansionCall(HvHashExpID, qwInpAddSize, qwInpProtBuf, qwInpDataBuf, qwOutHashBuf);
	}
